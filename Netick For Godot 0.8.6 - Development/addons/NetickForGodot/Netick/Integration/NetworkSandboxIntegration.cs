...
namespace Netick.GodotEngine
{
    ...
    public unsafe partial class NetworkSandbox : Node, IGameEngine
    {
        ...
        internal NetworkObject LinkPrefabChildForClient(NetworkObject root, int childIndex, int id, bool usable)
        {
            NetworkObject entity = root.BakedInternalPrefabChildren[childIndex] as NetworkObject;
            ...
            if (!Engine.Entities.ContainsKey(id))
            {
                ...
                {
                    Engine.ClientAddEntity(root.SpawnTick, entity, id, usable ? Engine.LocalPeer : null, true, false, rootSpawnKey);
                }

                else
                {
                    Engine.ClientAddEntity(root.SpawnTick, entity, id, usable ? Engine.LocalPeer : null, true, true, default);
                    Callbacks.OnObjectCreated(entity);
                }
            }
        ...
        private void InternalNetworkDestroy(Node obj)
        {
            ...
            if (entity != null)
            {
                ...
                if (entity.BakedInternalPrefabRoot == null)
                {
        ...
        private void UnlinkChildren(Node obj)
        {
            foreach (Node child in obj.GetChildren())
            {
                ...
                if (childNet != null)
                {
                    if (childNet.BakedInternalPrefabRoot == null)
        ...
        private void GetObjectResetInfo(NetworkObject entity, out bool shouldReset, out bool isPrefabRoot)
        {
            ...
            shouldReset = (entity.IsPrefabInstance || entity.BakedInternalPrefabRoot != null) && usePool;       //entity.InternalDestroy();   // TODO: add pool support  
            if (entity.PrefabId > -1 && entity.BakedInternalPrefabRoot == null && usePool)
                isPrefabRoot = true;
            isPrefabRoot = false;
        }
